...
| **Phase 0: v1.4 Retrofit** | **3.5-5 天** 🆕 | - | **33-38.5 天** | ⏳ **待開始** | **Sprint 3 前置：錯誤碼重配與 ETL 對齊** |
| Sprint 3: 特徵與模型 | **21-26 天** 🆕 | +1.5 天 | **55.5-66 天** | ⏳ 待開始 | +3天 (GNN + 物理損失 + Multi-Task) |
...
graph TD
    ...
    subgraph "Phase 0: v1.4 Retrofit 🔧 (Sprint 3 前 3.5-5 天)"
...
**狀態:** ⏳ **待開始**  
**阻擋條件:** 完成前不可啟動 Sprint 3 Feature Engineer v1.4
...
| 任務 ID | 任務描述 | 預估工時 | 優先級 | 驗收標準 | 狀態 |
|:---:|:---|:---:|:---:|:---|:---:|
| **IC-R01** | E750-E759 改為 GNN 拓樸錯誤 | 0.5天 | 🔴 Critical | 更新描述為拓樸相關 (E750: 缺少拓樸上下文) | ⏳ 待開始 |
| **IC-R02** | E800-E808 遷移至 E840-E848 | 0.5天 | 🔴 Critical | 原有 OPT 錯誤前綴改為 E84x | ⏳ 待開始 |
| **IC-R03** | 新增 E800-E829 CL 錯誤 | 0.5天 | 🔴 Critical | 定義 E800-E804 (觸發), E810-E820 (訓練), E815 (分散式鎖), E827-E828 (設備異動) | ⏳ 待開始 |
| **IC-R04** | 更新既有代碼引用 | 0.5天 | 🔴 Critical | 搜尋並替換所有 E801-E808 引用為 E841-E848 | ⏳ 待開始 |
| **IC-R05** | ERROR_CODES 字典註冊 | 0.3天 | 🔴 Critical | 新代碼加入字典，舊代碼標記棄用 | ⏳ 待開始 |
...
| 任務 ID | 任務描述 | 預估工時 | 優先級 | 驗收標準 | 狀態 |
|:---:|:---|:---:|:---:|:---|:---:|
| **FA-R01** | YAML SSOT 支援 topology 欄位 | 0.5天 | 🔴 Critical | Schema 支援 nodes/edges 定義 | ⏳ 待開始 |
| **FA-R02** | Excel 範本 v1.4 欄位規則 | 0.5天 | 🔴 Critical | topology, control_semantics 欄位可輸入 | ⏳ 待開始 |
| **FA-R03** | excel_to_yaml.py 升級 | 0.5天 | 🔴 Critical | 新欄位正確轉換至 YAML | ⏳ 待開始 |
| **IC-R06** | 檢查點 #7 CL 整合實作 | 0.3天 | 🔴 Critical | Training→CL→Optimization 契約實作 | ⏳ 待開始 |

### Phase 0.3 核心 ETL 管線升級 (v1.4 拓樸與型別貫通) (1.5-2 天)
...
| 任務 ID | 任務描述 | 預估工時 | 優先級 | 驗收標準 | 狀態 |
|:---:|:---|:---:|:---:|:---|:---:|
| **P-R01** | Parser 契約對齊 | 0.5天 | 🔴 Critical | 實作解析 `topology` 與 `control_semantics` 欄位，並避免 E103 觸發 | ⏳ 待開始 |
| **C-R01** | Cleaner 邏輯增強 (E35x) | 0.5-1天 | 🔴 Critical | 實作對齊 E75x 的防護邏輯，放行 GNN 特徵，確保設備檢測不誤殺 | ⏳ 待開始 |
| **BP-R01** | BatchProcessor 無損寫入 | 0.5天 | 🔴 Critical | 確保寫入 Parquet 時節點/邊緣陣列型別不遺失，且紀錄入 Manifest | ⏳ 待開始 |

**Phase 0 啟動條件與驗收檢查清單:**
- [ ] Phase 0.1: IC-R01~R05 完成 (配置與錯誤代碼重分配完成)
- [ ] Phase 0.2: FA-R01~R03 完成 (Excel 範本與 YAML Schema 支援拓樸)
- [ ] Phase 0.3: P-R01, C-R01, BP-R01 完成 (前段 ETL 順利吞吐 GNN 資料)
- [ ] 既有所有 Sprint 1 & 2 回歸測試全數通過 (Backward Compatibility)
...
#### 3.1 Feature Engineer v1.4 (7-9天) 🆕 +1-2天
...
**交付物:**
- `src/etl/feature_engineer.py` 🆕 **v1.4 拓樸感知版**
- `tests/test_feature_engineer_v14.py` 🆕

**關鍵驗收:**
- [ ] 直接查詢 AnnotationManager 取得 device_role
- [ ] 輸出 feature_order_manifest (E601)
- [ ] 輸出 scaler_params (E602)
- [ ] 🆕 **輸出 topology_context 含 adjacency_matrix** (E750-E759)
- [ ] 🆕 **輸出 control_semantic 特徵供 GNN 使用**
...
| **R-009a** | 🆕 **錯誤代碼重分配遺漏** | 🔴 High | Medium | **IC-R04 全面代碼掃描** | Phase 0 | ⏳ 待開始 |
| **R-009b** | 🆕 **既有 ETL 回歸測試失敗** | 🔴 High | Medium | **Phase 0.3 相容性驗證** | Phase 0 | ⏳ 待開始 |
...
| **M2.5: Phase 0 Retrofit** | **第 7 週初** 🆕 | - | IC-R01~R05, FA-R01~R03, P-R01, C-R01, BP-R01 | 錯誤代碼重分配完成，ETL 支援拓樸與型別 | ⏳ 待開始 |
| **M3: ML 就緒** | **第 10 週末** 🆕 | - | FE **v1.4**, Training **v1.4**, Hybrid, **GNN** | 檢查點 #4-#7 通過，**E750-E759** | ⏳ 待開始 |
...
| Feature Engineer | **v1.4.12** | 2026-02-26 | v1.2 | E305, E601-E602 | ⏳ Sprint 3 |
...
*最後更新: 2026-02-26 | 文件版本: v2.1-RC | 文件狀態: 🟢 Sprint 1-2 全部完成｜Sprint 3-5 首輪開發任務確立，PRD 已審查至生產黃金級標準*

**關鍵里程碑:**
- M3: ML 就緒 (第 10 週末) - 含 FE v1.4 + Training v1.4 + GNN
