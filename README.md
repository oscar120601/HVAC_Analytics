...
我們已完成從 v1.0 到 v1.3 的架構升級,文件庫 (`docs/`) 已全面更新至最新規範,包含 Interface Contract v1.1、Feature Annotation v1.2、以及所有核心模組的 v1.3 版本。核心程式碼 (`src/`) 正在按照 Foundation First Policy 逐步實施。
...
│   ├── features/               # [TODO] 特徵管理
│   │   └── annotation_manager.py # v1.2 Excel-to-YAML SSOT
│   ├── equipment/              # [TODO] 設備驗證
...
│       ├── base.yaml           # 基礎特徵定義
│       └── sites/              # 各案場特徵 (繼承 base)
├── docs/                       # 專案文檔 (全面更新至 v1.3)
│   ├── Interface Contract/     # ★ Interface Contract v1.1
│   ├── Feature Annotation Specification/ # Feature Annotation v1.2
...
- **[Feature Annotation Specification v1.2](docs/Feature%20Annotation%20Specification/PRD_Feature_Annotation_Specification_V1.2.md)** ⭐
  - Excel → YAML SSOT 機制
  - 繼承鏈機制 (base.yaml → site.yaml)
  - device_role 語意標註 (primary/backup/seasonal)
  - E406 同步檢查 (Excel/YAML 修改時間)
...
- **[Parser v2.1](docs/parser/PRD_Parser_V2.1.md)**
  - Header Standardization (snake_case)
  - 強制 UTC/ns 時間戳輸出
  - E1xx 錯誤處理

- **[Cleaner v2.2](docs/cleaner/PRD_CLEANER_v2.2.md)**
  - 語意感知清洗 (device_role 調整閾值)
  - Equipment Validation Precheck (E350)
  - 職責分離三層防護 (白名單 + Schema 淨化 + CI Gate)
  - E2xx 錯誤處理

- **[BatchProcessor v1.3](docs/batch_processor/PRD_BATCH_PROCESSOR_v1.3.md)**
  - Manifest 生成 (annotation_audit_trail + equipment_validation_audit)
  - Temporal Baseline 傳遞
  - E406 同步驗證與文件鎖
  - E3xx 錯誤處理

- **[Feature Engineer v1.3](docs/feature_engineering/PRD_FEATURE_ENGINEER_V1.3.md)**
...
- **[Model Training v1.3](docs/Model_Training/PRD_Model_Training_v1.3.md)**
...
- **[Chiller Plant Optimization Engine v1.2](docs/Chiller_Plant_Optimization_Engine/PRD_Chiller_Plant_Optimization_V1.2.md)**
...
- **[Hybrid Model Consistency v1.0](docs/Hybrid_Model_Consistency/PRD_Hybrid_Model_Consistency_v1.0.md)**
...
- **[Equipment Dependency Validation v1.0](docs/Equipment_Dependency_Validation/PRD_Equipment_Dependency_Validation_v1.0.md)**
  - ETL 階段物理邏輯一致性檢查
  - 設備依賴關係驗證
  - 歷史資料驗證

- **[Wizard Technical Blockade v1.0](docs/Wizard_Technical_Blockade/PRD_Wizard_Technical_Blockade_V1.0.md)**
  - Feature Annotation Wizard 技術封鎖機制
  - 防止並發修改與資料競態

### 📊 分析報告

- **[PRD 全面分析報告](docs/system_overview/PRD_Analysis_Report.md)** 🆕
  - 系統架構總覽與依賴圖
  - 10+ 核心模組詳細分析
...
- [ ] **Temporal Baseline** (時間基準機制)
  ...
  - [ ] E000 檢查機制
- [ ] **Feature Annotation v1.2** (YAML SSOT)
...
- **[Feature Annotation v1.2](docs/Feature%20Annotation%20Specification/PRD_Feature_Annotation_Specification_V1.2.md)** - YAML SSOT 機制
