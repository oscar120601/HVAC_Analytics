...
│   └── config/           # 配置系統
│       ├── feature_mapping.py       # 特徵映射配置 (V3)
│       ├── feature_mapping_v2.py    # 特徵映射 V2
│       └── mapping_editor.py        # CLI 映射編輯器
├── data/                  # 資料目錄
│   ├── CGMH-TY/          # 長庚醫院台北資料
...
在「⚡ 最佳化模擬」模式中使用視覺化界面配置特徵映射：

**位置**: ⚡ 最佳化模擬 → 🗺️ 特徵映射 (第一個分頁)
...
- **⚡ 最佳化模擬模式**: 
  - **🗺️ 特徵映射(新)**: 三種配置方式（自動識別、手動對應、萬用字元模式），支援 13 種 HVAC 物理系統層級特徵類別
  - **🎯 即時最佳化**: 調整變頻器參數，即時預估能耗與節能效益
  - **📊 特徵重要性**: 查看模型特徵重要性排名
  - **📈 歷史追蹤**: 追蹤過去最佳化結果
  - **🔧 模型訓練**: 訓練新模型與管理已訓練模型
